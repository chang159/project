...
include('function/connect_db.php');
	...
	$user=mysqli_query($con,"SELECT profile_id,title_id,profile_fname,profile_lname,profile_idcard,profile_nationality,profile_ethnicity,
		...
		FROM profiles AS em  INNER JOIN subdistricts AS sd ON (em.subdistrict_id=sd.id) INNER JOIN districts AS ds ON (sd.district_id=ds.id) WHERE profile_id='$_GET[id]'") or die ("Mysqli error profiles =>>>".mysqli_error($con));
    ...
    if($permission != 4){
      $username=mysqli_query($con,"SELECT username,passwd FROM USERS")or die ("Mysqli error profiles =>>>".mysqli_error($con));
      list($user_name,$user_pass)=mysqli_fetch_row($username);
    }
    
?>
	<!--ฟอร์มกรอกข้อมูลทนายความ-->
<h3 class='titleh'>แก้ไขข้อมูลทนายความ</h3>
<form action="index.php?module=2&action=7" method="post" class='table'>
...
<div class='shadow-sm p-3 mb-2 bg-white rounded border bg-light'>
    ...
    <label for="validationCustom05">ชื่อผู้ใช้งาน</label>
      <?php
        if($user_name ==""){
      ?>  
        <input type="text" class="form-control" id="formu" placeholder="Username" name="uname" required disabled value="<?php echo $user_name ?>">
      <?php
        }else{
      ?>
        <input type="text" class="form-control" id="formu" placeholder="Username" name="uname" required value="<?php echo $user_name ?>">
      <?php
      }
      ?>
    ...
    <?php
        if($user_pass ==""){
      ?>  
       <input type="password" class="form-control" id="formp" placeholder="Password" name="pwd" required disabled value="<?php echo $user_pass ?>">
      <?php
        }else{
      ?>
        <input type="password" class="form-control" id="formp" placeholder="Password" name="pwd" required value="<?php echo $user_pass ?>">
      <?php
      }
      ?>
...
<div class='shadow-sm p-3 mb-3 bg-white rounded border bg-light'>
    ...
    <div class="col-md mb-3">
      ...
      <select name="provinces" id='provinces' class="form-control " data-live-search="true"  data-size="6"
      required>
    	<?php
			//แสดงข้อมูลจังหวัด
...
<script type="text/javascript">
  $(document).ready(function() {
	 ...
	 loaddata($("#provinces option:selected").val(),<?php echo $districtid ?>,<?php echo $user_subdistrict_id ?>);
